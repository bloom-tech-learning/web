...
// const myFlavors = copy(originalFlavors);
// console.log('Task 1: ', myFlavors);
...
const updateFlavors = addFlavor(originalFlavors, "Rainbow Sherbert");

// console.log(updateFlavors);
...
function removeFlavorByName(arr, flavor){
  ...
  var index = arr.indexOf(flavor);
    if (index !== -1) {
      arr.splice(index, 1);
    }

  // for (let i = 0; i < arr.length; i++) {
  //   // return (arr[i].includes(flavor)) ? arr.splice(i, 1) : 'oop!, flavor not exist';
...
console.log(removeFlavorByName(updateFlavors, "Rocky Road"));
console.log(removeFlavorByName(updateFlavors, "Vanilla"));
...
for (let i = 0; i < arr.length; i++) {    
    // return (arr[i].includes(flavor)) ? arr.splice(i, 1) : 'oop!, flavor not exist';
       if (arr[i].includes(flavor)) {
          //  console.log(`index ${i}:`, arr[i]);
          flavorArr.push(arr[i]);
       }               
  }
return flavorArr;
}

console.log(removeFlavorByName(updateFlavors, "Chocolate"));
...
function getAverageWordLength(arr){
  ...
  let wordNum = [];
  for (let i=0; i < arr.length; i++) {
    wordNum.push(arr[i].split(" ").length);
  }
  return wordNum;
}

console.log('words numer in Array: ',getAverageWordLength(updateFlavors));
